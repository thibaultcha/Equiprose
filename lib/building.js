				...
				compile.compileMarkdownToFile(item, path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.buildDir, siteConfig.toJade, posts, function (err, pageFilePath) {
					...
					if (idx == pages.length - 1) {
						...
						posts.forEach(function (post, index, posts) {
							compile.compileMarkdownToFile(path.join(siteConfig.paths.posts.input, post.filename), path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.posts.output, siteConfig.toJade, posts, post.toJade, function (err, postFilePath) {
								if (err) return callback(err)
