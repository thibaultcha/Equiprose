...
var renderJade = function (layoutsDir, options, compress, callback) {
    ...
    var layoutPath = ''
    helpers.recursiveExists(options.layout + '.jade', layoutsDir, function (err, paths) {
        if (paths.length == 0) {
...
exports.compileMarkdownToFile = function (mdfile, config, posts, postMetas, callback) {
    ...
    renderJade(config.paths.templateDir, options, function (err, html) {
        ...
        fse.mkdirp(path.dirname(htmlFilePath), function (err) {
            if (err) return callback(err)
            fs.writeFile(htmlFilePath, emoji(html, path.join('/', path.basename(config.paths.assets.output), 'img/emojis'), 17), { encoding: 'utf-8' }, function (err) {
                if (err) return callback(err)
