...
exports.parseConfig = function (sitePath) {
    ...
    config.sitePath = path.resolve(sitePath)
    ...
    helpers.enumerateProperties(config.paths, function (prop) {
        ...
        if (firstChar !== path.sep && firstChar !== 'C:\\') {
            if (prop.match(/\.output$/)) {
                ...
                helpers.setAtPath(config.paths, prop, path.join(config.sitePath, helpers.pathToProperty(config.paths, prop)))
            }
        }
    })
...
var parseMetadatas = function (file) {
    ...
    if (!parsedMetas.slug || !/^[a-z0-9-]+$/.test(parsedMetas.slug)) {
        throw new Error('Provided slug is missing or invalid in: \''.white + metadatas.filename.red + '\''.white)
    }
...
exports.parsePostMetadatas = function (file, owner, dateFormat) {
    ...
    parsedMetas.toJade.author = parsedMetas.plainmetas.author || owner
    parsedMetas.toJade.date   = moment(new Date(plainMetas.date)).format(dateFormat)
