...
var compileMarkdown = function (mdfile, layoutsDir, toJadeConfig, callback) {
    if (!callback || typeof(callback) !== 'function') callback = function () {}
    ...
    jade.renderFile(layoutPath, { config: toJadeConfig, metas: filemetas.toJade }, function (err, html) {
        if (err) return callback(err)
...
exports.compileMarkdownToFile = function (mdfile, layoutsDir, buildDir, toJadeConfig, callback) {
    if (!callback || typeof(callback) !== 'function') callback = function () {}

    compileMarkdown(mdfile, layoutsDir, toJadeConfig, function (err, html, filemetas) {
        if (err) return callback(err)
        fs.exists(buildDir, function (exists) {
            if (!exists) {
                fse.mkdirsSync(buildDir)
