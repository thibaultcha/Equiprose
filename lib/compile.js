...
exports.compileStylusFile = function (stylusfile, buildDir, compress, callback) {
...
*/
var renderJade = function (layoutsDir, options, compress, callback) {
    if (arguments.length === 3) {
        callback = compress
        compress = true
    }
...
*/
exports.compileMarkdownToFile = function (mdfile, config, posts, postMetas, callback) {
    if (arguments.length === 4) {
        callback = postMetas
    }

    var filemetas = arguments.length === 5 ? postMetas : parse.parseMetadatas(mdfile)
    var postsToJade = []
    ...
    var options = {
        ...
        content : marked(filemetas.toJade.content).replace(/\n/, ''),
        config  : config.toJade,
        metas   : filemetas.toJade,
        posts   : postsToJade
    }

    renderJade(config.paths.templateDir, options, function (err, html) {
        if (err) return callback(err)
        
        // let's retrive the path with pages.input as root
        ...
        var regex = new RegExp("^([\\s\\S]*)\/" + config.paths.pages.input + "\/([\\s\\S]*)")
        var pathFromPagesInput = mdfile.match(regex)
        var htmlFilePath = path.join(config.paths.buildDir, path.dirname(pathFromPagesInput ? pathFromPagesInput[2] : ''), filemetas.slug + '.html')
