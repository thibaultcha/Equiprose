	...
	prepareOutputDir(siteConfig.paths.buildDir, siteConfig.paths.assets.input, siteConfig.paths.assets.output, function (err) {
		...
		compileStylesheets(siteConfig.paths.templateDir, siteConfig.paths.assets.output, function (err) {
			...
			pages.forEach(function (item, idx, pages) {
				compile.compileMarkdownToFile(item, path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.buildDir, siteConfig.toJade, function (err, pageFilePath) {
					if (err) return callback(err)
					
					if (idx == pages.length - 1) {
						// Posts
						posts.forEach(function (post, index, posts) {
							compile.compileMarkdownToFile(post, path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.posts.output, siteConfig.toJade, function (err, postFilePath) {
								if (err) return callback(err)
