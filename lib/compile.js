    ...
    */
    compileMarkdownToFile: function (mdfile, config, posts, postMetas, callback) {
        ...
        marked(filemetas.toJade.content, markedOpts, function (err, content) {
            ...
            self.renderJade(config.paths.templateDir, options, config.compress, function (err, html) {
                if (err) return callback(err)
                fse.mkdirp(path.dirname(htmlFilePath), function (err) {
                    if (err) return callback(err)
                    fs.writeFile(htmlFilePath
                                 , emoji(html, path.join('/', path.basename(config.paths.assets.output), 'img/emojis'), 17)
                                 , { encoding: 'utf-8' }
                                 , function (err) {
                        if (err) return callback(err)
                        callback(null, htmlFilePath, html, options)
                    })
                })
            })
        })
