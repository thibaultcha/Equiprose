...
module.exports = {
  ...
  */
  parseGlobalConfig: function () {
    ...
    var globalConfig = yaml.parse(data)
  ...
  */
  parseConfig: function (sitePath) {
    ...
    if (!fs.existsSync(path.join(sitePath, globalConfig.configFile))) {
      throw new Error('No ' + globalConfig.configFile + ' file at path: ' + path.join(sitePath, globalConfig.configFile))
    }

    var configStr   = fs.readFileSync(path.join(sitePath, globalConfig.configFile), { encoding: 'utf-8' })
    var config      = yaml.parse(configStr)
    config.sitePath = sitePath
      ...
      properties.enumerate(globalConfig, function (key, value) {
        var localProperty = properties.getAtPath(config, key)
        if (localProperty === null) {
          properties.setAtPath(config, key, value)
        }
      })

      // if buildDir is not absolute, get the relative path from sitePath
      if (config.paths.buildDir.charAt(0) !== path.sep) {
      ...
      properties.enumerate(config.paths, function (key, value) {
        if (value.charAt(0) !== path.sep) {
          if (/\.output/.test(key)) {
            properties.setAtPath(config.paths, key, path.join(config.paths.buildDir, value))
          }
          else {
            var propPath = path.join(config.sitePath, value)
            if (!fs.existsSync(propPath)) {
              throw new Error('Missing input directory: ' + propPath + ' for property: ' + key)
            }
            properties.setAtPath(config.paths, key, propPath)
          }
        }
      })
