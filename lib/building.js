...
exports.buildSite = function (sitePath, callback) {
	...
	var posts = []
	helpers.getFiles(siteConfig.paths.posts.input, new RegExp(/\.md$/), function (err, items) {
		if (err) return callback(err)
		posts = items
	})
	...
	prepareOutputDir(siteConfig.paths.buildDir, siteConfig.paths.assets.input, siteConfig.paths.assets.output, function (err) {
		...
		compileStylesheets(siteConfig.paths.templateDir, siteConfig.paths.assets.output, function (err) {
			...
			pages.forEach(function (item, idx, pages) {
				compile.compileMarkdownToFile(item, path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.buildDir, function (err, pageFilePath) {
					if (err) return callback(err)
					...
					if (idx == pages.length - 1) {
						...
						posts.forEach(function (post, index, posts) {
							compile.compileMarkdownToFile(post, path.join(siteConfig.paths.templateDir, 'layouts'), siteConfig.paths.posts.output, function (err, postFilePath) {
								if (err) return callback(err)
...
var fetchBlogPosts = function (config) {
    ...
    var name = config.toJade.owner.name || ''

    var posts = []
    dirContent.forEach(function (item) {
        posts.push(parse.parsePostMetadatas(path.join(config.paths.posts.input, item), name, config.dateFormat))
    })
    return posts
}
