    ...
    function emoji(someString, url, size) {
        return someString.replace(test, function (match) {
            if (emojis.indexOf(match) !== -1) {
                ...
                return '<span style="display:inline-block;background:url(' + url + '/' + encodeURIComponent(name) + '.png) no-repeat center center;background-size:'+size+'px;padding:0px 5px;height:'+size+'px;width:'+size+'px;vertical-align:-13%;"></span>'
            } else {
                return match;
            }
        });
