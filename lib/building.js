...
exports.buildSite = function (sitePath, callback) {
	// TODO: 
	// - create a metadatas object to pass to compileMarkdown
	//   containing 'config.toJade' ,'posts'.
	// - create a function to set config.paths to absolute paths.
	//   or do something else whatever instead of path.join everywhere.
	// - make all calls asynchronous ?
	// - use walk()...
	var siteConfig = parse.parseConfig(sitePath)

	var pages = []
	helpers.getFiles(path.join(siteConfig.sitePath, siteConfig.pages.input), new RegExp(/\.md$/), function (err, items) {
		if (err) return callback(err)
		pages = items
	})
	var posts = []
	helpers.getFiles(path.join(siteConfig.sitePath, siteConfig.posts.input), new RegExp(/\.md$/), function (err, items) {
		if (err) return callback(err)
		posts = items
	})
	...
	prepareOutputDir(siteConfig.buildDir, path.join(sitePath, siteConfig.assets.input), path.join(siteConfig.buildDir, siteConfig.assets.output), function (err) {
		...
		// Stylesheets
		compileStylesheets(path.join(siteConfig.sitePath, siteConfig.templateDir), path.join(siteConfig.buildDir, siteConfig.assets.output), function (err) {
			if (err) return callback(err)

			// Pages
			pages.forEach(function (item, idx, pages) {
				console.log('pages')
				compile.compileMarkdownToFile(item, path.join(siteConfig.sitePath, siteConfig.templateDir, 'layouts'), siteConfig.buildDir, function (err, pageFilePath) {
					if (err) return callback(err)
					...
					if (idx == pages.length - 1) {
						...
						posts.forEach(function (post, index, posts) {
							compile.compileMarkdownToFile(post, path.join(siteConfig.sitePath, siteConfig.templateDir, 'layouts'), path.join(siteConfig.buildDir, siteConfig.posts.output), function (err, postFilePath) {
								if (err) return callback(err)
